    ...
    Exchange.prototype._initialize = function () {
        ...
        this.initialized = new Promise(function (resolve, reject) {
            _this._connection.initialized.then(function () {
                _this._connection._connection.createChannel(function (err, channel) {
                    ...
                    if (err) {
                        ...
                        var callback = function (err, ok) {
                            /* istanbul ignore if */
                            if (err) {
                                exports.log.log("error", "Failed to create exchange '" + _this._name + "'", __assign({ module: "amqp-ts" }, getErrorDetails(err)));
                ...
                exports.log.log("error", "Channel failure, error caused during connection!", __assign({ module: "amqp-ts" }, getErrorDetails(err)));
            });
        });
                ...
                if (!_this._isConsumerInitializedRcp) {
                    ...
                    _this._channel.consume(DIRECT_REPLY_TO_QUEUE, function (resultMsg) {
                        ...
                        if (err) {
                            ...
                            _this._consumer_handlers.push([uuid, callback, resolve]);
                ...
                _this._connection._connection.createChannel(function (err, channel) {
                    ...
                    if (err) {
                        ...
                        var callback = function (err, ok) {
                            /* istanbul ignore if */
                            if (err) {
                                exports.log.log("error", "Failed to create queue '" + _this._name + "'", __assign({ module: "amqp-ts" }, getErrorDetails(err)));
                    ...
                    queue._channel.bindQueue(_this._destination._name, _this._source._name, _this._pattern, _this._args, function (err, ok) {
                        ...
                        if (err) {
                            exports.log.log("error", "Failed to create queue binding (" +
                                _this._source._name + "->" + _this._destination._name + ")", __assign({ module: "amqp-ts" }, getErrorDetails(err)));
